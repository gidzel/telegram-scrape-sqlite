...
def get_channel_messages(client, channel_name, limit=1, wait=False):
    import time
    from random import randrange
        ...
        if channel:#Todo: check for Channel? or Supergroup?
            try:
                with client.takeout() as takeout:
                    for message in takeout.iter_messages(channel):#client.iter_messages
                        if not limit:
                            break
...
def get_channel_messages_after_date(client, channel_name, date = None, wait=False):
    import time
    from random import randrange
    from telethon import errors
    messages = []
    try:
        ...
        if channel:#Todo: check for Channel? or Supergroup?
            try:
                with client.takeout() as takeout:
                    if date:
                        iter_m = takeout.iter_messages(channel, offset_date=date, reverse=True)
                    else:
                        iter_m = takeout.iter_messages(channel, reverse=True)
                ...
                with client.takeout() as takeout:
                    for message in client.iter_messages(channel):
                        ...
                        if message.fwd_from is not None:
                            if len(messages) and message.grouped_id != None and message.grouped_id == messages[-1].grouped_id:
                                messages[-1] = message
                            else:
                                messages.append(message)
                                limit -= 1
